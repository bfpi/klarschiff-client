class StartsController < ApplicationController
  def show
    if params[:advice]
      query = "?#{mobile_detected? ? 'request' : 'advice'}=#{ params[:advice] }"
      return redirect_client(query)
    end
    redirect_client
  end
  ...
  def frontend_url
    'http://demo.klarschiff-hro.de/'
  end

  def redirect_client(query = nil)
    return redirect_to "/map#{ query }" if mobile_detected?
    redirect_to "#{ frontend_url }#{ query ? "map.php#{ query }" : 'index.php' }"
  end
end
