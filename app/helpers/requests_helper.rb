...
module RequestsHelper
  def mark_trust(request)
    ...
    if trust.positive?
      tag.span(class: 'label label-default') do
        tag.span do
          trust.times do
            concat tag.i(nil, class: 'fas fa-star')
          end
        end
      end
    end
  ...
  def mark_photo_required(request)
    if request.extended_attributes.photo_required
      tag.i(nil, class: 'fas fa-camera', 'title' => t('requests.desktop.show.hint_photo_required'))
    end
  end
  ...
  def categories(type, current = nil)
    ...
    unless current
      categories.insert 0,
        [t('placeholder.select.category'), { disabled: true, selected: current.nil?, class: :placeholder }]
    end
  ...
  def d3_document_url(request)
    ...
    if res&.message&.include?('OK')
      ...
      street = 'nicht zuordenbar' if street.blank?
    else
