...
xml.rss version: '2.0', 'xmlns:atom': 'http://w3.org/2005/Atom', 'xmlns:georss': 'http://www.georss.org/georss' do
  xml.channel do
    ...
    @requests.each do |r|
      xml.item do
        xml.title "##{r.id} #{t(r.service.type, scope: 'service.types',
                                                count: 1)} (#{r.service.group} – #{r.service})"
        xml.description do
          html_cont = <<-HTML
          ...
          html_cont << '<br/>'
          html_cont << link_to(t('.link', name: Settings::Client.name),
                               Settings::Url.ks_server_url + map_path(request: r), target: '_blank', rel: 'noopener')
          xml.cdata! html_cont
        end
